...
void htlc::htlccreate(const uint64_t from, const uint64_t to, string hash_algorithm, const string preimage_hash, uint64_t preimage_size, uint64_t expiration) {
    uint64_t sender = get_trx_sender();
    ...
    insert_htlc(from, to, hash_algorithm, amount, preimage_hash, preimage_size, expiration, sender);
}
